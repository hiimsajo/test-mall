...
<details>
  <summary> 캐싱 적용 후 응답시간이 458ms에서 40ms로 약 <B>91.26% 감소</B> </summary>
...
- Kafka 기반 상품 등록 API 개선
    ...
    - 이미지 데이터를 DB에 저장하는 데 전체 API 수행시간의 약 <B>12%</B>를 차지.
    - Front가 있다는 가정하에 이미지 업로드 수행시간을 API에서 분리한다면, 이미지 데이터를 DB에 저장하는 데 전체 API 수행시간의 약 <B>29%</B>를 차지하게 됨.
